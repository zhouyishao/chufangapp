<template>
  <view class="app-page detail-page">
    <view class="topbar">
      ...
      <text class="topbar-title">调料详情</text>
      <view class="topbar-spacer" />
    </view>

    <view v-if="loading" class="state-card glass-card">
      <text class="state-title">正在加载调料详情</text>
      <text class="state-desc">从后端读取调料资料。</text>
    </view>

    <view v-else-if="error" class="state-card glass-card">
      <text class="state-title">调料详情加载失败</text>
    ...
    <template v-else-if="ingredient">
      <view class="hero glass-card">
        ...
        <view class="hero-copy">
          ...
          <text class="hero-desc">{{ ingredient.category?.name || '后台已配置调料资料' }}</text>
        </view>
      </view>
